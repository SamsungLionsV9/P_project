...
function DashboardContent() {
  ...
  useEffect(() => {
    const loadDashboardData = async () => {
      ...
      try {
        // 통계 데이터
        const statsRes = await fetch("http://localhost:8001/api/admin/dashboard-stats");
        ...
        // 일별 요청 데이터
        const dailyRes = await fetch("http://localhost:8001/api/admin/daily-requests?days=7");
  ...
  const maxModelValue = Math.max(...stats.popularModels.map((m) => m.value || 1), 1);
            ...
            <div className="line-chart-wrapper">
              ...
              >
                ...
                {dailyValues.map((v, i) => {
                  ...
                  const x = dailyValues.length > 1 ? (i / (dailyValues.length - 1)) * 100 : 50;
